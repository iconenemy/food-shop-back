...
class FoodSectionController {
    ...
    async update (req: TypedRequestBodyParams<{id: Types.ObjectId}, IUserUpdate>, res: Response){
        const { id } = req.params
        ...
        if (username) {
            ...
            if (userByUsername && userByUsername.id !== id) return res.status(400).json({message: 'This username is already in use. Please try different username.'})
        }

        if (email) {
            const userByEmail = await this.UserService.findUserByEmail({email})
            if (userByEmail && userByEmail.id !== id) return res.status(400).json({message: 'This email is already in use. Please try different email.'})
        }

        if (phone_number) {
            const userByPhone = await this.UserService.findUserByPhoneNumber({phone_number})
            if (userByPhone && userByPhone.id !== id) return res.status(400).json({message: 'This phone number is already in use. Please try different phone number.'})
        }
        ...
        if (password) {
            ...
            await this.UserService.updateUserById(id, { ...req.body })
        }    
